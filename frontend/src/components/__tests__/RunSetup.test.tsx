...
describe('RunSetup', () => {
  it('renders title', () => {
    render(<RunSetup onStart={vi.fn()} score={zeroScore} onReset={vi.fn()} onShowHelp={vi.fn()} />)
  ...
  it('calls onStart with className when Start Run clicked', async () => {
    const onStart = vi.fn()
    render(<RunSetup onStart={onStart} score={zeroScore} onReset={vi.fn()} onShowHelp={vi.fn()} />)
    await userEvent.click(screen.getByText(/berserker/i))
  ...
  it('calls onShowHelp when how-to-play button is clicked', async () => {
    const onShowHelp = vi.fn()
    render(<RunSetup onStart={vi.fn()} score={zeroScore} onReset={vi.fn()} onShowHelp={onShowHelp} />)
    await userEvent.click(screen.getByRole('button', { name: /how to play/i }))
    expect(onShowHelp).toHaveBeenCalledOnce()
  })

  it('inline how-to-play content is not rendered', () => {
    render(<RunSetup onStart={vi.fn()} score={zeroScore} onReset={vi.fn()} onShowHelp={vi.fn()} />)
    expect(screen.queryByText(/3 floors/i)).not.toBeInTheDocument()
  })

  it('shows score', () => {
    render(<RunSetup onStart={vi.fn()} score={{ runsCleared: 2, runsFailed: 5, bestRooms: 18 }} onReset={vi.fn()} onShowHelp={vi.fn()} />)
    expect(screen.getByText(/2 runs cleared/i)).toBeInTheDocument()
  })

  it('calls onReset when Forget me clicked', async () => {
    const onReset = vi.fn()
    render(<RunSetup onStart={vi.fn()} score={zeroScore} onReset={onReset} onShowHelp={vi.fn()} />)
